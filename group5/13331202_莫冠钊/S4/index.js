...
function get_random_array(len) {
	var book = [true, true, true, true, true];
	...
	while (count < 5) {
		var temp = Math.round(Math.random() * len);
		if (book[temp]) {
			count++;
			result.push(temp);
			book[temp] = false;
	...
	//在大气泡上显示字母顺序
	var mysum = document.getElementsByClassName('mysum')[0];
	mysum.innerHTML = letter[result[0]]+'、'+letter[result[1]]+'、'+letter[result[2]]+'\n'+
		letter[result[3]]+'、'+letter[result[4]];
...
function at_event(options) {
	...
	var random_next = options.random_array[index];

	button_event(buttons[random_next], options, function(){at_event(options);});
}
