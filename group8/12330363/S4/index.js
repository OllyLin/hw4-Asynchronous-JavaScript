window.onload = function() {
  ...
  document.getElementsByClassName("apb")[0].onclick = function() {
    ...
    showOrder(arr);
    var i = 0;
    var auto = function(arr, i) {
      if (i > 4)
        return;
      document.getElementsByClassName("button")[arr[i]].click();
      i += 1;
      setTimeout(function() { auto(arr, i) }, 3200);
    };
    auto(arr, i);
...
function showOrder(arr) {
  ...
  for (var i = 0; i < arr.length; i++)
    str += alpha[arr[i]];
  document.getElementById("show-order").textContent = str;
}
