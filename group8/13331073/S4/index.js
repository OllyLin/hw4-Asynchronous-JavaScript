...
function getRandomNumberFromServer() {
	...
	plus.onmouseover = function() {
		...
		info_bar_isclick = false;
		document.getElementsByTagName("p")[0].innerHTML = null;
		arr = [0, 1, 2, 3, 4];
	}
...
window.onload = function() {
	...
	document.getElementById("at_plus").onclick = doProgram_s4;
}

function doProgram_s4() {
	order = 0;
    getsort(); 
    document.getElementsByTagName("p")[0].innerHTML = tranfrom(arr[0]) +
                                                   tranfrom(arr[1]) +
                                                   tranfrom(arr[2]) +
                                                   tranfrom(arr[3]) +
                                                   tranfrom(arr[4]);
    document.getElementsByTagName("li")[arr[0]].click();
    order++;
...
function analog_click() {
	document.getElementsByTagName("li")[arr[order]].click();
	order++;
	if(order == 5) {
