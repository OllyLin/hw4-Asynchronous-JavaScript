...
window.onload = function() {
	var done = [];
	var sequences = [1, 2, 3, 4, -1];
	var btnGroup = [];
	getAllButton(btnGroup, sequences, done);
...
function getAllButton(group, seq, done) {
	var btns = document.getElementsByName('button');
	var boxes = document.getElementsByName('textbox');
	for (var i = 0; i < btns.length; i++) {
		var cur = new interactiveButton(btns[i], boxes[i], group, seq[i], done);
		cur.setClickEvent();
		group.push(cur);
	}
}
