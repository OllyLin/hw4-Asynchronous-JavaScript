...
function buttonInterface(buttons, p) {          //handler 
    ...
    xmlhttp.onreadystatechange=function() {             //得到服务器返回值后的操作
        if (xmlhttp.readyState==4 && xmlhttp.status==200) {
            unread.innerHTML = xmlhttp.responseText;        //灭活按钮
            ...
            if (on == true) document.getElementById('info-bar').onclick();           //当机器人程序启动时，继续自动操作
            else return;
        }
    }
...
function sumUp(buttons, info) {         //计算总和
    ...
    for (var i = 0; i < buttons.length; i++) {
        ...
        if (unread.innerHTML != "" && unread.innerHTML != "...") {
