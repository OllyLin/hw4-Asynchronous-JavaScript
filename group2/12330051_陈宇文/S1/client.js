  ...
  httpa.onreadystatechange=function()
  {
     if (httpa.readyState==4 && httpa.status==200)  {
		...
		if(count == 5) {
		  ...
		  temp.style["background-color"] = "blue";
		  temp.onclick = function(){calculate();}
		 }		 
     }
  }
  httpa.open("GET","/",true);
...
function init() {
  ...
  cira.onclick = function() {asyncA();};
  cirb.onclick = function() {asyncB();};
  circ.onclick = function() {asyncC();};
  cird.onclick = function() {asyncD();};
  cire.onclick = function() {asyncE();};
