window.onload = function() {
	...
	for (var i = 0; i < 5; i++) {
		...
		circles[i].onclick = get_number(false, circles, i, ev, function() {});
	}
...
function get_number(flag, circles, i, ev, callback) {
	return function() {
		// flag用于判断是否允许同时点击多个气泡
		if (flag == true) {
			get_num(flag, circles, i, ev, callback);
			return;
		}
...
function get_num(flag, circles, i, ev, callback) {
	circles[i].flag = 0;
	...
	ajax_get_number(flag, small_circle, circles, ev, i, callback);
}


function ajax_get_number(flag, small_circle, circles, ev, i, callback) {
	var xmlhttp;
...
function auto1(circles, ev) { // 仿真机器人
	return function() {
		get_number(true, circles, 4, ev, function() {})();
		get_number(true, circles, 3, ev, function() {})();
		get_number(true, circles, 2, ev, function() {})();
		get_number(true, circles, 1, ev, function() {})();
		get_number(true, circles, 0, ev, function() {})();
	};
}