...
function auto(rings, ev) {
	return function() {
		...
		var random_number = [0, 1, 2, 3, 4];
		random_number.sort(randomsort);


		get_number(rings, random_number[3],ev, function() {
			get_number(rings, random_number[2], ev, function() {
				get_number(rings, random_number[1], ev, function() {
					get_number(rings, random_number[0], ev, function() {
						get_number(rings, random_number[4], ev, get_sum)();
					})();
				})();
			})();
		})();
...
function number_from_server(liring, rings, ev, i, callback) {
	...
	xmlhttp.onreadystatechange = function() {
	 	if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
	 		...
	 		if (all_inactive) document.getElementById('ring5').dispatchEvent(ev);
