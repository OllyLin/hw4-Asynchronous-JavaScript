...
function robot(elem, RandomList, index) {
	if (index < 5) {
		if (elem[RandomList[index]].state == "isclick") {
		index++;
		robot(elem, RandomList, index);
		return;
		}
		if (elem[RandomList[index]].state == "noclick" && IsClass(elem[RandomList[index]], "enable")) {
			click_circles(elem[RandomList[index]]);
			var number;
			var that = elem[RandomList[index]];
			Ajax("http://localhost:3000", function(number) {
				getRandomNum(that, number);
				stateChanging(that);
				Check_Sum();
