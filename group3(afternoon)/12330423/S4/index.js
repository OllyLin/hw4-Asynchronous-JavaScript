...
function robotFiveFinger() {
    ...
    if (bigBubble.getElementsByTagName('span').length == 0) {
        orderSpan = document.createElement('span')
        bigBubble.appendChild(orderSpan);
    }

    order.sort(function () { return Math.random() > 0.5 ? -1 : 1;});

    for (var i = 0; i < 5; i++) {
        clickOneButton(order[i]);
        orderStr += (String(IDs[order[i]]) + ' ');
    }
    orderSpan = bigBubble.getElementsByTagName('span')[0];
    orderSpan.innerText = orderStr;
