...
function addUp(bar, aBubbles) {
  if (hasClass(bar, 'enable')) {
    ...
    var aNumbers = [];
    for (var i = 0; i < aBubbles.length; i++) {
      var number = aBubbles[i].getElementsByTagName('div')[0].innerHTML;
      sum += parseInt(number);
    }
...
window.onload = function() {
  ...
  //鼠标移出事件
  oBtn.onmouseout = function(ev) {
    var oEvent = ev || event;
    //避免移入子元素导致事件触发
    var reltg = oEvent.relatedTarget ? oEvent.relatedTarget : oEvent.toElement;  
