...
function mainclick(list) {
	...
	button_click(list, x, 0);
}

function button_click(list, x, i) {
	var unreads = document.getElementsByClassName("unread");
	alert(i);
	list[x[i]].style.backgroundColor = "gray";
	unreads[x[i]].style.visibility = "visible";
	unreads[x[i]].innerHTML = "...";
	var xmlHttpReg = null;
