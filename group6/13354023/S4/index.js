...
/* Pre: A state showing the stage of the simulation
...
function robot(done) {
  ...
  if (stage < 5) {
    buttons[order[stage++]].dispatchEvent(event);
  } else if (stage === 5) {
 ...
 * Post: The order is displayed in the page */
function displayOrder(order) {
  var display = ['A', 'B', 'C', 'D', 'E'];
  var element = addTag($('at-plus-container'), 'span');
  addClass(element, 'order');
  for (var i = 0; i < 5; i++) {
    if (i) element.innerHTML += '->';
    element.innerHTML += display[order[i]];
  }
}
...
function setCalculator() {
  ...
  $('at-plus-container').onmouseleave = function() {
    ...
    removeTag($('at-plus-container'), 'span');
    reset = true;
    robotMode = false;
  };
}
