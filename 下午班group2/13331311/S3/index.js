...
function getNextButton(thisButton) {
	...
	for (var i = 0; i < buttons.length; i++) {
		...
		if (i == target)
			if (buttons[target].getElementsByTagName("span")[0].style.opacity == 0)
...
function requestRamdomNumber(button) {
	...
	if (xmlhttp != null) {
	  	xmlhttp.onreadystatechange = function(button, xmlhttp) {
	  		return function() {
	  			if (!flag)
	  				return ;
	  			
	  			getRamdomNumber(button, xmlhttp);
	  		}
	  	}(button, xmlhttp);
...
function getRamdomNumber(button, xmlhttp) {
	if (xmlhttp.readyState == 4) {  // 4 = "loaded"
  		if (xmlhttp.status == 200) {  // 200 = "OK"
  			if (button.getElementsByTagName("span")[0].innerHTML == '...')
